...
def verify_claim(claim, pages):
    ...
    cited_page = next((p for p in pages if _same_page(p.get("retrieved"), cited)), None)
    if cited_page is not None:
        ...
        if v == "pass":
            return {"claim": claim["text"], "verdict": "grounded", "span": span,
                    "cited_page": cited, "support_checked": False, "why": ""}
    ...
    for p in pages:
        ...
        if v == "pass":
            return {"claim": claim["text"], "verdict": "misattributed", "span": span,
                    ...
                    "why": "span is verbatim but on a page the answer did not cite"}

    # 3) nowhere in the retrieved set
    return {"claim": claim["text"], "verdict": "unfounded", "span": span,
            "cited_page": cited, "why": "span does not occur verbatim in any retrieved page"}
